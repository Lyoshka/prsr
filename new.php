...
<?php
	...
	*/
	...
	$site[] = 'krasnojarsk.'; 		// Красоярск
	$site[] = '';					// Москва
	$site[] = 'novosibirsk.';		// Новосибирск
	$site[] = 'sankt-peterburg.';	//Санкт-Петербург
//	$site[] = 'belgorod.';
//	$site[] = 'penza.';
//	$site[] = 'habarovsk.';
//	$site[] = 'uljanovsk.';
...
//	$site[] = 'ekaterinburg.';
	...
	for ($j=0;$j<count($site);$j++) {
		...
		$html = curl_get('https://' . $site[$j] . 'megaflowers.ru/filters/all');
		$html_base = new simple_html_dom();
		$html_base->load($html);
		logger("Загрузили: " . 'https://' . $site[$j] . 'megaflowers.ru/filters/all');
	...
	for($i=0;$i<count($prod_arr);$i++) {	
			...
			// Обновление сотрировки
			$PID = check_new_XMLID( $IBLOCK_ELEMENT, $prod_arr[$i]->get_ID() );

			$SORT = 500 + $i;
			sort_update( $PID, $SORT );	
...
// *********************************************************************************************************************	
function check_new_price( $in_array, $sort ) {
	...
	if ( $PID )  {
		...
		//logger( "ID: " . $PID . " OLD PRICE MAIN: " . $price);
		
		// Сравниваем цену
		if ( $price != $in_array->get_Price() ) {
			
			update_price( $PID, $in_array->get_Price() );
			logger("Обновление цены ОСНОВНОГО букета: " . $in_array->get_Name() . " Новая цена: " . $in_array->get_Price());
		...
		if ( $PID )  {
			...
			//logger( "OLD PRICE 1: " . $price);
			if ( $price != $in_array->get_priceMin() ) {
				
				update_price( $PID, $in_array->get_priceMin() );
				logger("Обновление цены МАЛОГО   букета: " . $in_array->get_Name() . " Новая цена: " . $in_array->get_priceMin());
		...
		if ( $PID )  {
			...
			//logger( "OLD PRICE 2: " . $price);
			
			if ( $price != $in_array->get_priceMiddle() ) {
			
				update_price( $PID, $in_array->get_priceMiddle() );
				logger("Обновление цены СРЕДНЕГО букета: " . $in_array->get_Name() . " Новая цена: " . $in_array->get_priceMiddle());
		...
		if ( $PID )  {
			...
			//logger( "OLD PRICE 3: " . $price);
			
			if ( $price != $in_array->get_priceMaxi() ) {
			
				update_price( $PID, $in_array->get_priceMaxi() );
				logger("Обновление цены БОЛЬШОГО букета: " . $in_array->get_Name() . " Новая цена: " . $in_array->get_priceMaxi());
...
if (CModule::IncludeModule("iblock")):
	...
	if ( $PropID == false  ) {
		...
		if (  $IBLOCK_ID == $IBLOCK_PRODUCT ) {
			logger("Добавлено новое свойство: " . $attr_name);
		}
		return true;
	}
